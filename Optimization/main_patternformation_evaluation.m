...
%% Load data
folder = 'data/';
pattern_name = 'hexagon_lattice'; % Pattern evolved
n            = 1; % Number of evolutions stored
evaluate     = 1; % Evaluate. 1=evaluate. 0=use existing data-set.
...
if evaluate
    ...
    for i = 1:n
        for k = 1:numel(n_agents_vect)
            ...
            evo_p{i}.stats{k} = simulation_episode_batch_launch ( sml, evo_p{i}.Q2, n_episodes, 'visualize',1,'verbose', 1);
            fprintf('Saving...\n\n')
            if save_dataset
                make_folder(folder);
